...
class AgentLoop {
  constructor(instance, emit) {
    ...
    // PvP: зелья + отступление
    this._potionsUsed = false;
  ...
  async _drinkPotion(namePattern) {
    const bot = this.bot;
    const potion = bot.inventory.items().find(i =>
      i.name?.includes(namePattern) &&
      !i.name?.includes("splash") &&
      !i.name?.includes("lingering")
    );
  ...
  async _useCombatPotions() {
    if (this._potionsUsed) return;
    this._potionsUsed = true;
    const bot = this.bot;
    if (!bot?.entity) return;
    try { await bot.equip(await this._findBestWeapon(), "hand").catch(() => {}); } catch {}
    await this._drinkPotion("strength");
    await this._drinkPotion("speed");
  }

  async _retreatFromCombat() {
    const bot = this.bot;
    ...
    if (pearl) {
      try {
        ...
        bot.activateItem();
        log.info("[AgentLoop] Retreat: throwing ender pearl");
        return;
      } catch {}
    }

    // Sprint away in opposite direction
    try {
      const yaw = bot.entity.yaw + Math.PI + (Math.random() - 0.5) * 0.8;
      const dx = Math.sin(-yaw) * 18, dz = Math.cos(-yaw) * 18;
      const tx = Math.round(bot.entity.position.x + dx);
      const tz = Math.round(bot.entity.position.z + dz);
      const ty = Math.round(bot.entity.position.y);
      if (bot.pathfinder?.movements) bot.pathfinder.movements.allowSprinting = true;
      await bot.pathfinder.goto(new goals.GoalBlock(tx, ty, tz));
    } catch {} finally {
      if (bot.pathfinder?.movements) bot.pathfinder.movements.allowSprinting = false;
    }
  }

  async _findBestWeapon() {
    const bot = this.bot;
    const TIERS = ["netherite_sword","diamond_sword","iron_sword","stone_sword","wooden_sword","golden_sword","netherite_axe","diamond_axe","iron_axe"];
    ...
    try {
      if (dist > 3.2) {
        ...
        if (now - this._lastAttackTime >= this._nextAttackDelay) {
          ...
          // Пьём зелья при первом столкновении
          this._useCombatPotions().catch(() => {});

          // Ест яблоко / еду при HP < 10 в бою
          if ((bot.health || 20) < 10) {
          ...
          // Jump-крит: прыжок → удар во время падения = 1.5x урон
          // Делаем примерно в 60% случаев (не каждый удар, как у живого игрока)
          const doCrit = bot.entity.onGround && Math.random() < 0.6;
          if (doCrit) {
            bot.setControlState("jump", true);
            await delay(60 + Math.random() * 60);
            bot.setControlState("jump", false);
            await delay(80); // падаем
          }
  ...
  async _onHealthTick() {
    ...
    // ── Отступление: HP < 5 и есть цель ──────────────────────────────
    const hp = bot.health ?? 20;
    if (hp < 5 && this._combatTarget && !this._retreating &&
        now - this._lastRetreat > 15000) {
      this._retreating = true;
      this._lastRetreat = now;
      log.info("[AgentLoop] HP critical, retreating!");
      try { await this._retreatFromCombat(); } catch {}
      this._retreating = false;
      return;
    }

    // ── Исцеление: выпить зелье при HP < 8 ────────────────────────────
    if (hp < 8 && now - this._lastEat > 3000) {
      const healed = await this._drinkPotion("healing").catch(() => false);
      if (healed) { this._lastEat = now; return; }
    }
