...
export default function CenterPanel({ bot }: Props) {
  if (!bot) {
    return (
      <div className="panel flex-1 flex items-center justify-center"
        style={{ color: "#555", background: "rgba(26,26,26,0.85)" }}>
        <div className="text-center">
          <div className="text-4xl mb-3">⛏️</div>
          <p className="text-xs">Выберите бота или создайте нового</p>
          <p className="text-xs mt-1" style={{ color: "#444" }}>
            Вкладка «Боты» → + Добавить бота
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="panel flex-1 flex flex-col overflow-hidden"
      ...
      {/* Body — stats + inventory only (controls are in LeftPanel) */}
