...
- wait: подождать 2 секунды`;
  ...
  async _actCollectBlock(bot, blockName) {
    ...
    for (const id of candidates) {
      ...
      if (block) {
        // FIX: GoalNear(2) вместо GoalBlock — бот стоит РЯДОМ с блоком, не пытается залезть в него.
        // GoalBlock для брёвен выше головы заставлял pathfinder карабкаться на уровень Y блока,
        // что вызывало хаотичное вращение головы («дёргается как наркоман»).
        // FIX walk: для блоков выше головы pathfinder не умеет забираться вертикально
        // → используем Y бота, чтобы навигация была горизонтальной (всегда успешна).
        // После движения бот стоит рядом и копает ВВЕРХ — так гораздо надёжнее.
        const navY = block.position.y > bot.entity.position.y + 1.5
          ? bot.entity.position.y   // блок выше — идём на свой уровень
          : block.position.y;       // блок рядом — идём к нему
        await bot.pathfinder.goto(
          new goals.GoalNear(block.position.x, navY, block.position.z, 2)
        ).catch(() => {});

        // Останавливаем pathfinder и вручную прицеливаемся перед ломкой
        try { bot.pathfinder.stop(); } catch {}
        await this._sleep(80);

        const refreshed = bot.blockAt(block.position);
        if (refreshed && refreshed.name !== "air") {
          // FIX aim: bot.dig(forceLook=true) использует canSeeBlock mineflayer
          // который даёт false для блоков СТРОГО ВЫШЕ головы → dig отвергается сервером.
          // Вручную вычисляем прицел в правильную грань и lookAt(force=true).
          const _eyeY = bot.entity.position.y + 1.62;
          const bpos  = refreshed.position;
          const faceY = _eyeY > bpos.y + 1.02 ? 0.91  // смотрим вниз на верхнюю грань
                      : _eyeY < bpos.y - 0.02 ? 0.09  // смотрим вверх на нижнюю грань
                      : 0.50;                           // смотрим на боковую грань
          try { await bot.lookAt(bpos.offset(0.5, faceY, 0.5), true); } catch {}
          await this._sleep(50);
          await bot.dig(refreshed).catch(() => {}); // без forceLook — уже прицелились
