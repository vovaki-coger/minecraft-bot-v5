...
class AnarchyBrain {
  ...
  async _trainAsync() {
    ...
    try{
      ...
      let iterDone=0;const TOTAL=800;
      await this.net.trainAsync(data,{
        iterations:TOTAL,errorThresh:0.005,logPeriod:80,
        log:(s)=>{iterDone+=80;prog(Math.min(20+Math.round((iterDone/TOTAL)*72),92),`⚡ Итерация ${iterDone}/${TOTAL}`);}
      });
